...
class DQN(rl_agent.AbstractAgent):
  ...
  def add_transition(self, prev_time_step, prev_action, time_step):
    ...
    if current_player == -4:
      current_player = 1
    prev_player = prev_time_step.observations['current_player']
    legal_actions = (time_step.observations["legal_actions"][current_player])
    legal_actions_mask = np.zeros(self._num_actions)
    legal_actions_mask[legal_actions] = 1.0
    transition = Transition(
        info_state=(
            prev_time_step.observations["info_state"][prev_player][:]),
        action=prev_action,
        reward=time_step.rewards[current_player],
        next_info_state=time_step.observations["info_state"][current_player][:],
