...
def exec_py(model_load_dir, save_dir, loop_num, num_players, num_dices, num_dice_sides):
  
  # 定义要执行的Bash命令或脚本
  predict_training_py =  '/repo/open_spiel/python/examples/liars_card_predict_example.py'
  equil_calc_py =  '/repo/open_spiel/python/examples/liars_equil_use_predict_example.py'
  # 预测信息保存文件名
  predict_log_name = f"predict_{FLAGS.checkpoint_dir[16:]}.log"
  predict_save_dir = os.path.join(save_dir, predict_log_name)
  # 执行 预测网络训练Python 脚本
  process = subprocess.Popen(['python3', predict_training_py, model_load_dir, str(num_players), str(num_dices), str(num_dice_sides)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
  ...
  # 执行 预测网络训练Python 脚本
  process = subprocess.Popen(['python3', equil_calc_py, model_load_dir, str(num_players), str(num_dices), str(num_dice_sides)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
  output, error = process.communicate()
  ...
  with tf.Session() as sess:
    ...
    for ep in range(FLAGS.num_train_episodes):
      if (ep + 1) % FLAGS.save_every == 0:
        ...
        exec_py(save_dir_with_loop, save_dir, ep+1, num_players, num_dices, num_dice_sides)
