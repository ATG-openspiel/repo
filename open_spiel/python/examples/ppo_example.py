...
flags.DEFINE_float("learning_rate", 1.5e-4,
                   "the learning rate of the optimizer")
flags.DEFINE_integer("seed", 1, "seed of the experiment")
flags.DEFINE_integer("total_timesteps", 300_000_000,
...
def main(_):
  ...
  for update in range(num_updates):
    for _ in range(FLAGS.num_steps):
      ...
      #玩家i行动前其他玩家随机采取动作
      time_step = agent.pre_step(time_step, envs)

      #玩家i行动
      agent_output = agent.step(time_step)
      time_step, _, _, unreset_time_steps = envs.step(
          agent_output, reset_if_done=False)
    
      #玩家i行动后到下次玩家i行动前或玩家i行动后到游戏结束
      time_step = agent.post_step(time_step, envs, recent_rewards)

      # for ts in time_step:
      #   if ts.last():
      #     real_reward = ts.rewards[player_id]
      #     writer.add_scalar(f"charts/player_{player_id}_training_returns", real_reward,
