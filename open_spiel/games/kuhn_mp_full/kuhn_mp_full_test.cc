...
void BasicKuhnTests() {
  testing::LoadGameTest("kuhn_mp_full");
  testing::ChanceOutcomesTest(*LoadGame("kuhn_mp_full"));
  testing::RandomSimTest(*LoadGame("kuhn_mp_full"), 100);
  testing::RandomSimTestWithUndo(*LoadGame("kuhn_mp_full"), 1);
  for (Player players = 3; players <= 5; players++) {
    testing::RandomSimTest(
        *LoadGame("kuhn_mp_full", {{"players", GameParameter(players)}}), 100);
  }
}

void CountStates() {
  std::shared_ptr<const Game> game = LoadGame("kuhn_mp_full");
...
int main(int argc, char **argv) {
  open_spiel::kuhn_mp_full::BasicKuhnTests();
  open_spiel::kuhn_mp_full::CountStates();
  open_spiel::kuhn_mp_full::PolicyTest();
  open_spiel::testing::CheckChanceOutcomes(*open_spiel::LoadGame(
      "kuhn_mp_full", {{"players", open_spiel::GameParameter(3)}}));
  open_spiel::testing::RandomSimTest(*open_spiel::LoadGame("kuhn_mp_full"),
                                     /*num_sims=*/10);
  open_spiel::testing::ResampleInfostateTest(
      *open_spiel::LoadGame("kuhn_mp_full"),
      /*num_sims=*/10);
}
