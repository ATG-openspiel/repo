...
#include "open_spiel/games/liars_dice_info/liars_dice_info.h"
...
const GameType kGameType{
    /*short_name=*/"liars_dice_info",
    /*long_name=*/"Liars Dice Info",
...
std::string LiarsDiceState::InformationStateString(Player player) const {
  ...
  std::string result;
  std::vector<std::string> all_players_dice_results;

  //表示玩家的骰子信息
  //玩家0只知道自己的信息；其他玩家知道互相的信息
  if(player == 0){
    result = absl::StrJoin(dice_outcomes_[player], "");
  }else{
    for (int i = 1; i < num_players_; i++) {
      all_players_dice_results.push_back(absl::StrJoin(dice_outcomes_[i], ""));
    }
    result = absl::StrJoin(all_players_dice_results, " ");
  }
...
void LiarsDiceState::InformationStateTensor(Player player,
  ...
  SPIEL_CHECK_EQ(values.size(), num_players_ +
                                    (num_players_ * max_dice_per_player_ * dice_sides()) +
  ...
  if(player == 0){
    ...
    offset += (num_players_ - 1)*max_dice_per_player_ * dice_sides();
  }else{
    offset += max_dice_per_player_ * dice_sides();
    for(int i = 1; i < num_players_; i++){
        int my_num_dice = num_dice_[i];
        for (int d = 0; d < my_num_dice; d++) {
          int outcome = dice_outcomes_[i][d];
          if (outcome != kInvalidOutcome){
            SPIEL_CHECK_GE(outcome, 1);
            SPIEL_CHECK_LE(outcome, dice_sides());
            values[offset + (outcome - 1)] = 1;
  ...
  offset = num_players_ + num_players_ * max_dice_per_player_ * dice_sides();
...
std::vector<int> LiarsDiceGame::InformationStateTensorShape() const {
  ...
  // 编码合法动作的(total_num_dice_ * dice_sides_)位
  // 编码calling liar玩家的1位
  return {num_players_ + (num_players_ * max_dice_per_player_ * dice_sides_) +
